...
public class IRepoHandler extends IHandler<HttpRequest> {
    ...
    public IRepoHandler(Connector connector) throws ConnectorException {
        this(connector.getPackage());
    }
    ...
    public IRepoHandler(Context<Package> aDatabase, String aRepoName,
            List<Service<Resource<?>, ValidationResult>> postValidators) {
    ...
    @Override
    protected void onExecute(Action aAction, HttpRequest aRequest) throws Exception {
        try {
    ...
    protected void onPost(Action aAction, HttpRequest aRequest) throws IOException {
        Transaction tx = theRepo.connect(aAction.getActor());
