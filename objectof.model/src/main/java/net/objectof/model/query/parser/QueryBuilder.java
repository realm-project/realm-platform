...
public class QueryBuilder extends QueryParserBaseVisitor<Object> {
	...
	@Override public Operator visitOperator(@NotNull QueryParser.OperatorContext ctx) { 
		switch (ctx.getText()) {
	...
	@Override public Query visitBracketedQuery(@NotNull QueryParser.BracketedQueryContext ctx) {
		return (Query) visit(ctx.theQuery);
	}
	
	
	@Override public Query visitCompositeQuery(@NotNull QueryParser.CompositeQueryContext ctx) { 
	...
	@Override public Query visitRelationship(@NotNull QueryParser.RelationshipContext ctx) {
	...
	@Override public ILoadQuery visitRetrieval(@NotNull QueryParser.RetrievalContext ctx) { 
		String key = ctx.theField.getText();
	...
	@Override public Relation visitRelation(@NotNull QueryParser.RelationContext ctx) { 
		switch (ctx.getText()) {
			case "=": return Relation.EQUAL;
			case "!=": return Relation.UNEQUAL;
			case "contains": return Relation.CONTAINS;
			case "<=": return Relation.LTE;
			case ">=": return Relation.GTE;
	...
	@Override public Resource<?> visitId(@NotNull QueryParser.IdContext ctx) {
		String kind = ctx.theKind.getText();
		String label = visit(ctx.theLabel).toString();
		return tx.retrieve(kind, label);
	}
	
	@Override public Long visitInteger(@NotNull QueryParser.IntegerContext ctx) {
		return Long.parseLong(ctx.getText());
	}
	
	@Override public Double visitReal(@NotNull QueryParser.RealContext ctx) { 
		return Double.parseDouble(ctx.getText());
	}
	
	@Override public String visitString(@NotNull QueryParser.StringContext ctx) {
		return ctx.theString.getText();
	}
