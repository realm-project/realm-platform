...
public class ISourcePackage extends IPackage
{
  ...
  protected Map<String, Property> processAttributes(Kind<?> aType,
      Element aElement)
      {
  ...
  protected void processElement(IKind<?> aParent, Element aElement)
  {
    ...
    if (stereotype == null)
    {
      throw new RuntimeException("Invalid element '" + aElement.getTagName()
          + "': No Stereotype defined.");
    }
    String selector = aElement.getAttribute("selector");
    String pathname = (aParent != null ? aParent.getComponentName() + "." : "")
        + selector;
