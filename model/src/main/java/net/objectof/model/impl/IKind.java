...
public class IKind<T> extends IPeer<T> implements Kind<T>, Member {
    ...
    public IKind<?> findMember(String aSelector) {
        for (IKind<?> kind : getParts()) {
            if (kind.getSelector().equals(aSelector)) { return kind; }
        }
    ...
    public Object invoke(Invocation aMessage) {
        ...
        if (isInstance(r.getValue())) { return r.getValue(); }
     ...
     */
    public void setProperties(Map<String, Property> aProperties) throws IllegalStateException {
        if (theProperties != null) { throw new IllegalStateException("Properties are already defined."); }
