...
public class TestRealmRepo {
	...
	public static void createRepo() {
		...
		IPackage schema = new ISourcePackage(IBaseMetamodel.INSTANCE, new File(
				"src/main/resources/packages/realm.xml"));
		ISqlDb db = new ISqlDb("stressTestDatabase");
		Package repo = db.createPackage(repositoryName,
				IRip.class.getName(), schema);
...
//		return db.getPackage("test.objectof.org:1407/test/");
	...
	static void createCourseTeachers(Transaction aTransaction,
			Course aCourse, int index)
	{
//		Create teachers
		@SuppressWarnings("unchecked")
		IIndexed<Person> teachers = (IIndexed<Person>) aTransaction
...
//		Create the teachers and assign it:
	...
	private static void createPerson(Transaction t, String name, String email, String pwdHashed, String salt, String role) {
		...
		if (role.compareTo("teacher") == 0)
			person.setRole(t.retrieve("Role", "2"));
		else 
			System.out.println("createPerson: Unknown role!");
	}
...
//
//			@SuppressWarnings("unchecked")
//			IMapped<Position> positions = (IMapped<Position>)t.create("Command.outputPositions");
//
//			for (int j = 0; j < 5; j ++) {
//				Position position = (Position)t.retrieve("Position", Integer.toString((i + j) % 10));
...
//	}
	...
	private static void createSession(Transaction t) {
		...
		@SuppressWarnings("unchecked")
		IMapped<Device> devices = (IMapped<Device>) t.create("Session.devices");
