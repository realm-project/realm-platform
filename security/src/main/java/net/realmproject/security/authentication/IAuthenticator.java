...
/**
 * Handles authentication of a user when logging in. Expects a JSON Payload described by the LoginInfo class
 * @author nathaniel
 *
 */
...
public class IAuthenticator extends AbstractAuthenticator<LoginInfo> {
	...
	protected void onAuthenticate(Action action, HttpRequest request, Person person, LoginInfo info, boolean success)
		...
		if (success) {
			...
			//get a session, or create a new one, and store the Person record in it.
