...
/**
 * Decorates transactions in order to work with databases which can only use a
 * single connection at a time. With all transactions using a single connection,
 * all operations such as post, query, etc... should be coordinated with
 * synchronized blocks
 * 
 * @author NAS
 *
 */
class ISingleConnectionTxDecorator implements Transaction {

    private Transaction backer;
    private Object lock;

    public ISingleConnectionTxDecorator(Transaction backer, Object globalLock) {
        this.backer = backer;
